...
let middle_of_triple trojica = 
  let (prvi, drugi, tretji) = trojica in 
  drugi 
...
let starting_element seznam = 
  let gl::_ = seznam in 
  gl
(*kako se doda napako? v tem primeru naredis match in locis dva primera*) 
...
let rec sum_int_pairs seznam =
  if seznam = [] then 
    ...
    let gl::rep = seznam in 
    let (pr, dr) = gl in 
    pr + dr :: sum_int_pairs rep
...
let rec insert x k list =
  if k < 0 then 
    x :: list    
  else if List.length list < k then 
    list @ [x]
  else
    let gl::rep = list in 
    match k with 
    | 0 -> x :: list
    | n -> gl :: insert x (k-1) rep
...
let rec divide k list = 
  if list = [] then 
    ...
    if k = 0 then 
      ...
      let (gl2, rep2) = divide (k-1) rep in
      (gl :: gl2, rep2)
...
let rec is_palindrome list = 
  ...
  |[_] -> true
  |_ ->
    let list2 = obrat list in
    let gl1::rep1 = list in
    let gl2::rep2 = list2 in
    if gl1 = gl2 then 
      let nov1 = obrat rep1 in 
      let a::nov = nov1 in 
      is_palindrome nov
    else 
      false
...
let rec max_on_components list1 list2 = 
  if list1 = [] || list2 = [] then 
    ...
    let gl1::rep1 = list1 
    and gl2::rep2 = list2 in
    max gl1 gl2 :: max_on_components rep1 rep2
...
let rec second_largest list = 
  let nov = remove (najvecja list) list in 
  najvecja nov