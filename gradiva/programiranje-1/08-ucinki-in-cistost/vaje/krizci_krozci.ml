...
let winner_of_list triples : player option = 
  let rec pomozna acc =
    ...
    | [] -> acc
    | gl::rep -> 
      match (winner_of_triple gl) with 
      | None -> pomozna acc rep
      | _ -> winner_of_triple gl
  in pomozna None triples
...
let place_token player grid (row_i, col_i) : state =
  ...
  | None (* grid is full *) ->
      ...
      GameOver {result = Remi; final_grid = empty_grid}
  | Some player ->
      (* Dobili smo zmagovalca, torej vrnemo [GameOver] z novo mrežo in
      zmagovalcem *)
      GameOver {result = Zmaga (player); final_grid = empty_grid}
...
let show_state = function
  ...
  | GameOver {result; final_grid} ->
      ...
      function 
      | Remi -> "Remi"
      | Zmaga (player) -> "Zmaga" ^ show_player player
      in
...
let rec loop state = 
  ...
  match state with
  | GameOver (result, grid) -> print_string ()
  | OnTurn (player, grid) -> 
